...
Plik obejmuje: rejestrację/login, first-run onboarding po założeniu konta, wizard onboardingu (Źródło → Pytania), skip onboardingu, powrót do uzupełnienia danych po skipie/przerwaniu oraz opcję "powiadom nas o brakującej aplikacji".

Realny stan UI 29.04.2026: wizard ma 2 fazy (Źródło → Pytania). Nie ma w wizardzie kroku "przegląd zaimportowanych treningów". Nie ma w wizardzie propozycji stref HR. Po submit/skip user idzie do dashboardu. Nowy user po rejestracji trafia od razu do onboardingu, a po skipie/przerwaniu widzi później opcję kontynuacji z Dashboardu lub Profilu.
...
- Frontend pokazuje bazowy przełącznik Logowanie / Załóż konto.
- Stan po EP-005: rejestracja działa w UI, zbiera `username`, email do resetu hasła oraz hasło min. 8 znaków. Nadal do domknięcia: pełny smoke produkcyjny, zgody i docelowy kod duplikatu 409.

### Kroki użytkownika
1. Wchodzi na stronę.
2. Widzi formularz logowania.
3. Klika "Załóż konto".
4. Podaje login, email, hasło i powtórzenie hasła.
...
- Po submit: konto utworzone, automatyczny login, redirect do first-run onboardingu.
- First-run onboarding jest domyślnym następnym krokiem po rejestracji, ale user może go pominąć bez utraty dostępu do aplikacji.

### Stan po EP-002 / EP-005
- UI ma przełącznik Logowanie / Załóż konto.
- Formularz rejestracji ma email używany przez reset hasła.
- Po rejestracji frontend zapisuje sesję i ustawia `onboardingCompleted = false`, więc user trafia do wizardu.
- Nadal do domknięcia: login jako email albo jasna decyzja `username + email`, zgody przy rejestracji, smoke produkcyjny i docelowy kod duplikatu 409 zamiast obecnego 422.

### Oczekiwane API
- `POST /api/auth/register` — body: `{ username, email, password }` → tworzy użytkownika i sesję.
...
- Jeśli user zamknie/pominie onboarding, Dashboard/Profil pokazuje później CTA "Dokończ onboarding" albo "Uzupełnij dane".
...
## US-ONBOARD-007 — Powrót do onboardingu z Dashboardu lub Profilu

**Typ:** happy path / recovery
**Persona:** P-NOVICE (po skipie)
**Status:** implemented (2026-04-30: zakładka Profil ma realny formularz edycji przez ProfileEditSection.tsx; CTA powrotu do onboardingu (OnboardingReturnCta) nadal dostępne)
**Priorytet:** P0

### Stan wejściowy
User wcześniej pominął first-run onboarding, przerwał go, albo zrobił tylko część. Chce uzupełnić dane po czasie.

### Preconditions
- User zalogowany.
- Dashboard i zakładka Profil istnieją jako bazowe widoki.
- CTA powrotu do onboardingu nie jest jeszcze wdrożone.

### Kroki użytkownika
1. Widzi CTA "Dokończ onboarding" na Dashboardzie albo w Profilu.
2. Klika CTA.
3. Widzi swoje obecne dane, jeśli cokolwiek wcześniej zapisał.
4. Otwiera się wizard lub formularz edycji z prefill obecnych wartości.
5. Edytuje, submituje.

### Oczekiwane zachowanie UI
- CTA jest widoczne dla usera z `onboardingCompleted = false` albo profilem oznaczonym jako skipped.
...
### Kryteria akceptacji (P0)
- User po skipie nie musi zakładać nowego konta ani czyścić danych, żeby wrócić do onboardingu.
- CTA powrotu jest dostępne co najmniej z Dashboardu; docelowo również z Profilu.
...
### Testy / smoke
- Test e2e: register → skip onboarding → dashboard → dokończ onboarding → reload → wartości przetrwały.

### Uwagi produktowe
To jest para do first-run onboardingu po rejestracji: onboarding powinien startować od razu dla new user, ale nie może być jednorazową bramką. Jeśli user go pominie, zamknie kartę lub wróci po tygodniu, aplikacja musi spokojnie prowadzić go do uzupełnienia danych.

Stan po EP-003: bazowy powrót istnieje. CTA renderuje się na Dashboardzie i w Profilu, a wizard dostaje `initialProfile` i prefilluje zapisane pola. Do domknięcia zostaje manualny/e2e smoke oraz upewnienie się, że każdy partial update zachowuje nietknięte pola profilu.
