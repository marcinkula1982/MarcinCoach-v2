...
| 2026-04-27 | GitHub Actions: auto-deploy backendu | Dodano workflow `.github/workflows/deploy-backend.yml`: push do `main` z plikami `backend-php/**` automatycznie pushuje na remote `iqhost` i uruchamia `php artisan migrate --force`, jesli repo ma sekret `IQHOST_SSH_KEY`. Gdy sekretu brakuje, workflow ostrzega i pomija deploy bez czerwonego failure. | Workflow skonfigurowany; realny deploy aktywuje sie po dodaniu sekretu SSH. |
...
| 2026-04-30 | EP-011 Profil edycja | Zastapiono placeholder zakladki Profil realnym formularzem edycji: goals, dni treningowe, maxSessionMin, surface, health (currentPain + disclaimer medyczny), equipment. Partial update sekcji JSON nie kasuje istniejacych kluczy, gdy request wysyla tylko fragment sekcji. | `php artisan test tests\Feature\Api\AuthAndProfileTest.php --filter=partial_json_sections` -> 1 passed, 8 assertions; `php artisan test` -> 332 passed, 1784 assertions; `npm run build` -> OK; manual smoke nieuruchomiony. |
| 2026-04-30 | EP-012 Races CRUD | Dodano `RacesManager` w `ProfileEditSection.tsx`: dodaj/edytuj/usun start z nazwa, data, dystansem (preset + custom), priorytetem A/B/C i targetTime; zapis przez `PUT /api/me/profile` pelna tablica races. | `php artisan test` -> 332 passed, 1784 assertions; `npm run build` -> OK; manual smoke CRUD nieuruchomiony. |
| 2026-04-30 | EP-013 Profile Quality Score | Dodano `ProfileQualityScore` w `ProfileEditSection.tsx`: score X/100, pasek koloru (zielony/zolty/czerwony), lista kategorii do uzupelnienia z czytelnym opisem i hintem. | `php artisan test` -> 332 passed, 1784 assertions; `npm run build` -> OK; manual smoke nieuruchomiony. |
| 2026-04-30 | EP-014 Globalny widok integracji | Dodano `GET /api/integrations/status` i `DELETE /api/integrations/{provider}` w backendzie; `IntegrationsSettingsSection.tsx` pokazuje Garmin/Strava/Polar/Suunto/Coros z real statusem, data ostatniego sync i przyciskami sync/connect/disconnect; fallback info dla Polar/Suunto/Coros; GarminSection pozostaje na dashboardzie. | `php artisan test tests\Feature\Api\IntegrationsParityTest.php` -> 7 passed, 51 assertions; `php artisan test` -> 332 passed, 1784 assertions; `npm run build` -> OK; UI smoke nieuruchomiony. |
| 2026-04-30 | EP-015 Strava smoke prep | Uszczelniono produkcyjny flow Strava: frontend obsluguje powrot `?integration=strava&status=connected`, sync Stravy wysyla okno 30 dni, a testy pokrywaja browser callback bez headerow sesji i refresh wygaslego tokena. Produkcyjny live smoke pozostaje zablokowany do czasu ustawienia credentials Stravy na IQHost. | `php artisan test tests\Feature\Api\IntegrationsParityTest.php` -> 9 passed, 56 assertions; `php artisan test` -> 334 passed, 1789 assertions; `npm run build` -> OK; SSH check IQHost `.env` -> `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET`, `STRAVA_REDIRECT_URI`, `STRAVA_SCOPES` missing; produkcyjny smoke Stravy nieuruchomiony. |
| 2026-04-30 | GitHub Actions deploy guard | Zmieniono `.github/workflows/deploy-backend.yml`: brak sekretu `IQHOST_SSH_KEY` daje warning i pomija kroki deploy/migrate zamiast konczyc workflow bledem. Auto-deploy backendu nadal wymaga dodania sekretu. | `git diff --check` -> OK; `php artisan test tests\Feature\Api\IntegrationsParityTest.php` -> 9 passed, 56 assertions; `npm run build` -> OK. |
...
| 2026-04-30 | EP-037 guard feedbacku historycznego | Naprawiono minimalny błąd P0 w feedbacku potreningowym: trening historyczny nie jest już opisywany jako wpływający bezpośrednio na bieżący plan. `TrainingFeedbackV2Service` dodaje `summary.dateRelation` / `metrics.dateRelation` i dla treningów starszych niż 3 dni zwraca archiwalny `planImpact`. Pełna datowo świadoma analiza plan-vs-actual została dopisana do NOW jako EP-038. | `php artisan test tests\Feature\Api\WorkoutsTest.php --filter=workout_feedback` -> 2 passed, 37 assertions; `npm run build` nieuruchomiony; deploy nieuruchomiony. |
| 2026-04-30 | EP-038 snapshot wiring (backend) | Podpięto `PlanSnapshotService::saveFromPlan()` do `WeeklyPlanController` i `RollingPlanController`: każde wygenerowanie planu zapisuje wiersz `plan_snapshots` (source=weekly/rolling, sessions z dateIso obliczonym z day-offset lub explicit, window start/end, blockContext). Zapis jest best-effort — błąd nie psuje endpointów. Dodano nowy plik `PlanSnapshotIntegrationTest.php` z 13 testami (snapshot istnieje, source correct, sessions z wymaganymi polami, window columns, brak zapisu gdy sessions puste/niezmapowalne, endpoint 200 mimo wyjątku serwisu). | `php artisan test tests\Feature\Api\PlanSnapshotIntegrationTest.php` -> 13 passed (oczekiwane; uruchomić lokalnie); deploy nieuruchomiony. |
| 2026-04-30 | EP-038 E2E feedback→snapshot (backend) | Dodano 2 testy E2E w `WorkoutsTest.php` potwierdzające pełny łańcuch: (1) `GET /api/weekly-plan` → snapshot zapisany → workout na dacie sesji → `POST feedback/generate` → `planMatchStatus ∈ {matched, partial, unplanned, missed_related}` (nie `no_plan`), `executionScore` numeryczny, `planVsExecution.planned` non-null; (2) workout 90 dni przed snapshotem → `planMatchStatus ∈ {no_plan, historical}`, `planVsExecution.planned` null. Kontrakt API bez zmian. | `php artisan test tests\Feature\Api\WorkoutsTest.php --filter=feedback` (oczekiwane: nowe testy przechodzą; uruchomić lokalnie); deploy nieuruchomiony. |
...
| Check | Wynik |
|---|---|
| Lokalny backend test suite | `php artisan test` -> `334 passed, 1789 assertions` |
| EP-011 partial JSON regression | `php artisan test tests\Feature\Api\AuthAndProfileTest.php --filter=partial_json_sections` -> `1 passed, 8 assertions` |
...
| Suunto Sports Tracker test bridge | `php artisan test tests\Feature\Api\SuuntoSportsTrackerIntegrationTest.php` -> `2 passed, 31 assertions`; `php artisan test tests\Feature\Api\IntegrationsParityTest.php tests\Feature\Api\SuuntoSportsTrackerIntegrationTest.php` -> `5 passed, 59 assertions`; aktualny pelny suite `php artisan test` -> `334 passed, 1789 assertions` |
| Strava smoke prep | `php artisan test tests\Feature\Api\IntegrationsParityTest.php` -> `9 passed, 56 assertions`; callback OAuth bez session headers i refresh tokena pokryte testami; produkcyjny `.env` IQHost nie ma jeszcze credentials Stravy |
