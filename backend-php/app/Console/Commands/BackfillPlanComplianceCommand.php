...
class BackfillPlanComplianceCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'Generate PlanCompliance v1 for existing workouts';

    /**
     * Execute the console command.
     */
    public function handle(PlanComplianceService $service): int
    {
     ...
     */
    private function handleSingleWorkout(PlanComplianceService $service, int $workoutId): int
    {
        ...
        $this->info("✓ Generated compliance for workout ID: {$workoutId}");
     ...
     */
    private function handleBatch(PlanComplianceService $service): int
    {
        ...
        $query = DB::table('workouts')
            ->leftJoin('plan_compliance_v1', 'workouts.id', '=', 'plan_compliance_v1.workout_id')
            ->whereNull('plan_compliance_v1.workout_id')
            ->select('workouts.id')
        ...
        if (empty($workoutIds)) {
            $this->info('No workouts found that need compliance generated.');
            return Command::SUCCESS;
        }
        ...
        $this->info("✓ Completed. Generated compliance for {$processed} workout(s).");
